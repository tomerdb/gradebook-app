  ...
  db = {
    run: (sql, params = [], callback = () => {}) => {
      ...
      let pgSql = sql
        ...
        .replace(/TEXT DEFAULT CURRENT_TIMESTAMP/g, 'TIMESTAMP DEFAULT CURRENT_TIMESTAMP')
        .replace(/INSERT OR IGNORE/g, 'INSERT')
        .replace(/IF NOT EXISTS/g, '');
      
      // Handle INSERT OR IGNORE by converting to INSERT ... ON CONFLICT DO NOTHING
      if (sql.includes('INSERT OR IGNORE')) {
        // This is a simplified conversion - may need refinement for specific cases
        pgSql = sql.replace('INSERT OR IGNORE', 'INSERT') + ' ON CONFLICT DO NOTHING';
      }
    ...
    get: (sql, params = [], callback) => {
      let pgSql = sql.replace(/LIMIT 1/g, '');
      if (!pgSql.includes('LIMIT')) {
        pgSql += ' LIMIT 1';
      }
